...
async def get_graph(limit: int = 100):
    ...
    async with get_db() as db:
        result_lst = await db.read_transaction(work)
        ...
        for record in result_lst:
            ...
            node_lst.append({"title": record["movie"], "label": "movie"})
            target = i
            i += 1
            ...
            for name in record["cast"]:
                ...
                try:
                    source = node_lst.index(actor)
                except ValueError:
                    node_lst.append(actor)
                    source = i

                    i += 1
            
                rel_lst.append({"source": source, "target": target})

        return {"nodes": node_lst, "links": rel_lst}
...
async def get_search(q: Optional[str] = None):
    ...
    async with get_db() as db:
        ...
        result_lst = await db.read_transaction(work, q)
        return [serialize_movie(record["movie"]) for record in result_lst]
